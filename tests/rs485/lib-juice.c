...
int rj232_havechar(void)
{
    return(rj_readstat() & RXA232);
}
...
int rj232_putc(unsigned char c)
{
    return rj_writebyte(RS232D, c);
}

int rj232_read(unsigned char *buf, int maxlen)
{
    int i = 0;
    while (rj232_havechar() && (i < maxlen)) {
	buf[i++] = rj232_getc();
    }
    buf[i] = 0;
    return i;
...
int rj485_havechar(void)
{
    return(rj_readstat() & RXA485);
}
...
int rj485_putc(unsigned char c)
{
    return rj_writebyte(RS485D, c);
}

int rj485_read(unsigned char *buf, int maxlen)
{
    int i = 0;
    while (rj485_havechar() && (i < maxlen)) {
	buf[i++] = rj485_getc();
    }
    buf[i] = 0;
    return i;
